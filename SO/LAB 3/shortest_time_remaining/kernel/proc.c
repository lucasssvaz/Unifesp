 ...
 */
  int q = 7;	 		/* scheduling queue to use */
  ...
  if (!rdy_head[q]) {		/* add to empty queue */	
  	...
  	current_proc = rdy_head[q];
  	while(current_proc != rdy_tail[q]){
  		if(current_proc->p_cpu_time_left < current_proc->p_nextready->p_cpu_time_left){
  			if(current_proc == rdy_head[q]){
  				rp->p_nextready = rdy_head[q];
  				rdy_head[q] = rp;
  				insert_done = 1;
  				break;
  			} else {
  				rp->p_nextready = current_proc->p_nextready;
  				current_proc->p_nextready = rp;
  				insert_done = 1;
  				break;
  			}
  		} else {
