...
void Trie_Insert(Trie *Tree, int Key){
	int dig = 0, aux, Hold;
	Node *CurrentNode;
...
int Trie_Remove(Node *CurrentNode, int Key, int dig){
	int Search = 0, rem = 0;
	...
	if (GetDigit(Key,dig)==0){
		rem = Trie_Remove(CurrentNode->Left,Key,dig+1);
		if (rem==1){
			CurrentNode->Left=NULL;
		}
	} else {
		rem = Trie_Remove(CurrentNode->Right,Key,dig+1);
		if (rem==1){
